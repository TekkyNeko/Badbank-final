    ...
    if (err) {
      ...
      if (user) {
        if (user2 !== null && user.username !== user2.username) {
          ...
          let newBalance = Number(user.accounts.checking.balance) - Number(transfer);
          user.accounts.checking.balance = newBalance;
          

          let recipientBal = Number(user2.accounts.checking.balance) + Number(transfer);
          user2.accounts.checking.balance = recipientBal;
